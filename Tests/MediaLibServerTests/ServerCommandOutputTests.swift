...
final class ServerCommandOutputTests: XCTestCase {
    func testUsageDescribesCurrentAuthenticatedLoopbackBoundary() {
        ...
        XCTAssertTrue(ServerCommandOutput.usage.contains("HTTP Range、HLS"))
        XCTAssertTrue(ServerCommandOutput.usage.contains("桌面多用户与逐资料库授权、安全审计"))
        XCTAssertTrue(ServerCommandOutput.usage.contains("Web 列表/详情/播放、逐用户续播/已看、只读管理"))
        ...
        XCTAssertTrue(ServerCommandOutput.usage.contains("HLS 与分级限速"))
