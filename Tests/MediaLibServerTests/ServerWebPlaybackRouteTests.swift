...
final class ServerWebPlaybackRouteTests: XCTestCase {
    ...
    func testPlayerScriptUsesAuthorizedLifecycleWithoutUnsafeHTMLOrTokenStorage() {
        ...
        XCTAssertTrue(script.contains("const pendingHLSSessionIDs = new Set()"))
        XCTAssertTrue(script.contains("pendingHLSSessionIDs.add(provisionalSessionID)"))
        XCTAssertTrue(script.contains("for (const sessionID of pendingHLSSessionIDs) cancelHLSSession(sessionID, true)"))
    ...
    func testMobileTransportDropsTimeBeforeControlsOverflowAndUsesTapToggle() {
        ...
        XCTAssertTrue(script.contains("overlayInteractionActive"))
        XCTAssertFalse(script.contains("overlayPinnedByTap"))
        ...
        XCTAssertTrue(style.contains(".player-control-row { flex-wrap: nowrap; gap: 0; }"))
        XCTAssertTrue(style.contains("#seek-backward, #seek-forward { display: none; }"))
        XCTAssertTrue(style.contains("flex: 0 1 var(--control-height-lg)"))
